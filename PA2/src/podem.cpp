...
ATPG::wptr ATPG::find_pi_assignment(const wptr object_wire, const int &object_level) {
  ...
  if (object_wire->is_input()) {
    ...
    const nptr& n = object_wire->inode.front();
    switch (n->type) {
      case OR:
      case NAND:
        new_object_wire = object_level ? find_easiest_control(n) : find_hardest_control(n);
        break;
      case AND:
      case NOR:
        new_object_wire = object_level ? find_hardest_control(n) : find_easiest_control(n);
        break;
      case BUF:
      case NOT:
        new_object_wire = n->iwire.front();
        break;
      default:
        fprintf(stderr, "Oops\n");
        exit(-1);
...
bool ATPG::trace_unknown_path(const wptr w) {
  ...
  if(w->is_output()) return TRUE;
  for(nptr& n: w->onode) {
    const wptr& w = n->owire.front();
    if(w->value == U) {
      if(trace_unknown_path(w)) return true;
    }
  }
  return false;
  /* end TODO */
...
int ATPG::set_uniquely_implied_value(const fptr fault) {
  ...
  const int opposite_value = (fault->fault_type == STUCK0);
  switch(backward_imply(w, opposite_value))
  {
      case TRUE:
        pi_is_reach = TRUE;
        break;
